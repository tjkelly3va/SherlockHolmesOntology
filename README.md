...
**Deductions and methods as a context graph.** In knowledge graph architecture, a *context graph* is a subgraph that captures not just what is known, but how it came to be known — the provenance of reasoning. Together, the deductions and methods pillars form a two-layer context graph within the broader ontology. The methods layer records how evidence was analyzed: what technique was applied, by whom, to what evidence, for what investigative purpose, and whether it succeeded. The deductions layer records how conclusions were derived: what was observed, the logical chain connecting observation to conclusion, and whether the conclusion was ultimately validated or refuted. The `ex:supportsDeduction` bridge between the two layers completes the derivation chain — from physical evidence, through analytical technique, to reasoned conclusion, with outcome verification. When a user asks "How did Holmes figure out that the hat belongs to an intellectual?", the context graph provides a structured answer that traces the full path: the evidence examined (the hat), the method applied (physical object examination), the observations made, the reasoning chain, and the conclusion reached — each element a queryable node rather than a paragraph to parse.
