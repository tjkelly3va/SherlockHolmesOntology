...
**A three-layer context graph.** In knowledge graph architecture, a *context graph* is a subgraph that captures not just what is known, but how it came to be known — the provenance of reasoning. Three of the ontology's pillars — events, methods, and deductions — together form a three-layer context graph within the broader knowledge graph. The narrative events layer provides temporal and causal scaffolding: *when and why* an investigation happened, through the sequenced event chain with its motivation–action–outcome structure. The methods layer records the analytical process: *how* evidence was analyzed, by whom, using what technique, for what investigative purpose, and whether it succeeded. The deductions layer records the reasoning: *what* was concluded from the analysis, through what logical chain, and whether the conclusion was ultimately validated or refuted. The bridges between layers — `ex:techniqueEmployedIn` linking methods to their narrative events, and `ex:supportsDeduction` linking methods to the deductions they enabled — complete a derivation chain that is not merely linear but circular: a narrative event motivates an investigation, during which a technique is applied to evidence, producing findings that feed a deduction, whose conclusion is confirmed or refuted by a later narrative event. The provenance of any conclusion can be traced backward through all three layers to its narrative origin. When a user asks "How did Holmes figure out that the hat belongs to an intellectual?", the context graph provides a structured answer that traces the full path: the narrative event where the examination occurred, the evidence examined (the hat), the method applied (physical object examination), the observations made, the reasoning chain, and the conclusion reached — each element a queryable node rather than a paragraph to parse.

![Three-Layer Context Graph](ContextGraph.svg)
*The three-layer context graph illustrated with The Adventure of the Speckled Band. Layer 1 (blue) shows the narrative event chain — Holmes's investigation at Stoke Moran sits within a sequenced plot progression linked by `ex:precedes`. Layer 2 (teal) shows the investigative methods applied during that event — scene examination, ventilator analysis, and object identification — connected upward via `ex:techniqueEmployedIn`. Layer 3 (purple) shows the deductions those methods enabled — the murder mechanism and the inheritance motive — connected via `ex:supportsDeduction`. The dashed return arrow (`ex:confirmedBy`) closes the loop: Roylott's death in the resolution event confirms the deduction from Layer 3, completing the circular derivation chain.*
